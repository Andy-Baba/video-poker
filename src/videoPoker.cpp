...
const shi sec = 0;	//Seconds to interupt
const shi col = 6;	//the background color of the dialog box
const shi text = 2;   //the text color of the dialog box

struct cart {
	shi num;
	shi kin;
};
//Structure to define cards

cart deck[52], hand[5]; //define a deck of cards and a hand of cards

shi handp;	//hand pointer, points to the current position of the deck
...
//*************************************The functions doing the GUI work*****************************************

//Draws an empty frame with given width and height on the given position
void fram(int colour, int x, int y, int w, int h)
{
	register shi i, j;
	//textattr(colour);
	move(x,y);
	/*gotoxy(x, y);mvaddch(x,y,218);
	cprintf("%c", 218);
	for (i = 0; i < w; i++)
		cprintf("%c", 196);
	cprintf("%c", 191);
